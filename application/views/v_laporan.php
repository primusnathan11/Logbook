...
<div class="panel">
        ...
        <h2 class="display-4">Data Laporan </h2>
		 <br>
		 <hr class="style1">
		 <br>
		</div>
	 ...
	 ?>
	 <form action="<?=base_url('index.php/Laporan')?>" method="POST">
	 <div class="col-sm-4 inputtgl">
	 <span style="margin-left:-15px;"> Cari data dari tanggal</span> <br>
	 <input placeholder="Cari data dari tanggal" type="date" name="tglmin" class="inputdate" min="2019-01-01" max="2030-12-31">
	 </div>
	 <div class="col-sm-4 inputtgl">
	 <span style="margin-left:-15px;"> Sampai tanggal </span><br>
	 <input placeholder="Sampai tanggal" type="date" name="tglmax" class="inputdate"  min="2019-01-01" max="2030-12-31"> 
	 </div>
	 <br>
	 <div class="col-sm-4 inputtgl">
	 <input type="submit" name="simpan" value="Cari" class="btn btn-primary" style="margin-left:-15px;">
	 </div>
	 </form>
	 <?php 
	 }
	 ?>
			<table class="table table-striped table-hover" id="LaporanTabel" style="width:100%;">
			    <thead>
					<tr> 
                        <th>No</th>
						<th>ID Laporan</th>
        ...
        <?php $no=1; ?>
				...
				<tr>
					...
					<td>
						<?= $dt_kom->nama_teknisi? $dt_kom->nama_teknisi: "-"; ?>
					</td>
					<td> 
						<?= $this->m_komplain->userSpesifik($dt_kom->id_user)->nama; ?>
					</td>
					<td>
						<?= $dt_kom->detail? $dt_kom->detail: "-"; ?>
					</td>
					<td>
						<?= $dt_kom->tgl_activity? $dt_kom->tgl_activity: "-"; ?>
					</td>
					<td>
					<?= $dt_kom->tgl_selesai? $dt_kom->tgl_selesai: "-"; ?>
					</td>
					<td>
						<?= $dt_kom->keterangan? $dt_kom->keterangan: "-"; ?>
					</td>
					<?php if($this->session->userdata('nama_level') =='Teknisi'){?>
					<td>
					<a href="<?php echo base_url().'index.php/Komplain/kerjakan_komplain/'.$dt_kom->id_activity?>" class="btn btn-primary">Kerjakan</a>
					</td>
					<?php
					}
					?>
